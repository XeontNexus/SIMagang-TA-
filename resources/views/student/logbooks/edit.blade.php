...
@section('title', 'Edit Logbook - SIMagang')
    ...
    <div class="col-md-8">
        <div class="card shadow">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Logbook</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('student.logbooks.update', $logbook) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('minggu_ke') is-invalid @enderror" id="minggu_ke" name="minggu_ke" required>
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('minggu_ke', $logbook->minggu_ke) == $i ? 'selected' : '' }}>Minggu Ke-{{ $i }}</option>
                                @endfor
                            </select>
                            @error('minggu_ke')
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('bulan') is-invalid @enderror" id="bulan" name="bulan" required>
                                ...
                                @foreach($bulanList as $index => $namaBulan)
                                    <option value="{{ $index + 1 }}" {{ old('bulan', $logbook->tanggal_mulai->month) == ($index + 1) ? 'selected' : '' }}>{{ $namaBulan }}</option>
                                @endforeach
                            </select>
                            @error('bulan')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control bg-light" id="tahun" value="{{ $logbook->tanggal_mulai->year }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('kegiatan') is-invalid @enderror" id="kegiatan" 
                                  name="kegiatan" rows="3" required>{{ old('kegiatan', $logbook->kegiatan) }}</textarea>
                        @error('kegiatan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('hasil') is-invalid @enderror" id="hasil" 
                                  name="hasil" rows="2">{{ old('hasil', $logbook->hasil) }}</textarea>
                        @error('hasil')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('kendala') is-invalid @enderror" id="kendala" 
                                  name="kendala" rows="2">{{ old('kendala', $logbook->kendala) }}</textarea>
                        @error('kendala')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('solusi') is-invalid @enderror" id="solusi" 
                                  name="solusi" rows="2">{{ old('solusi', $logbook->solusi) }}</textarea>
                        @error('solusi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
